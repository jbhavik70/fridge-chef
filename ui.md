...
- A persistent navigation bar (`.tab-navigation`) splits the application into two views:
  1. **Grandma's Kitchen:** The ingredient entry console and recipe result card.
  2. **My Reservations:** The dashboard showing booked cooking sessions.
- **Active State:** The active tab is colored in Terracotta (`--color-primary`) and features an animated bottom border line.

### 2.3 Chef's Console (Ingredient Entry)
...
- **Meal Type Segment Selector:** A pill-shaped segmented selector where the active button slides into a warm cream base.
- **Dietary Preference Chips:** Multi-select pill chips that toggle to Sage Green (`--color-secondary`) when selected.

### 2.4 Warn / Notice States
...
- Displays an **animated Giphy preloader** depicting a boiling pot with steam rising (`.loader-gif-container`).
